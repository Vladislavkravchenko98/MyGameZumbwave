...
var d_speed = 100      ####Скорость коотрая включается если Зомби обноружил игрока 

var true_die = false ######################## Перменная отвечающая за то жив или мертв Зомби


##Переменные кот определют что зомби может атаковать
var bite_strength = 15  # Сколько HP БУДЕТ ОТНИМАТЬ ЗОМБИ У ИГРОКА ПРИ ОДНОМ УКУСЕ
...
func searc_for_target():          #поиск цели
	...
	if pl: ## Если есть кого доставать (чтобы игра не ломалась после смерти ГГ)
		if target and true_die == false:
			if global_position.distance_to(target.global_position) > 4500:      
				cancel_movement()
			else:
				set_destination(target.position)               #################СЮДА ПЕРЕДАЕТСЯ ПОЗИЦИЯ КУДА ИДТИ ....target.position
		elif global_position.distance_to(pl.global_position) < 4500:      #Проверка, что игрок именно близко, тк target может быть не всегда
			cancel_movement()
...
func ran_it():
	...
	var probability_of_occurrence = int(rand_range(0, 3))   #Вероятность появления лута
	if probability_of_occurrence == 2:
		for i in range(1):
			randomize()
