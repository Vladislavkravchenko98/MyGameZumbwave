...
func _process(delta):
	...
	elif allzm <=2 and pl == 1 and yoy == true and vl == 3:
		var inc = 0
...
var klo = false # нужна чтобы не дать появиться следующей волне по завершению игры
...
func Addzombi():
	...
	if number_of_wave == 1: #################################################### 1
		...
		if vl == 1:
			...
			vid5= true
			$Timer.start(1)
	elif number_of_wave >= 13:          
		klo = true
		UI.WaveLabeloff()
		UI.Winonn()
		#Сюда запихнуть надпись и телепорт на первую карту (кнопку)
		UI.End12wave()
