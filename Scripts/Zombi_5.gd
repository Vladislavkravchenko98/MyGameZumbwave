...
var d_speed = 300      ####Скорость коотрая включается если Зомби обноружил игрока 

var true_die = false ######################## Перменная отвечающая за то жив или мертв Зомби


##Переменные кот определют что зомби может атаковать
var bite_strength = 25  # Сколько HP БУДЕТ ОТНИМАТЬ ЗОМБИ У ИГРОКА ПРИ ОДНОМ УКУСЕ
...
func set_destination(dest):            ####Создание пути для Зомби
	if true_die == false and inc == true: ##################################################
		...
		if velocity.x > 0 and abs(dest.x - position.x) > abs(dest.y - position.y) and true_die == false:   #abs(dest.x - position.x) > abs(dest.y - position.y) and true_die == false:
			...
			###### Байт
			$BiteArea/DD1.disabled = true
			$BiteArea/DD2.disabled = true
			$BiteArea/DD3.disabled = true
			$BiteArea/DD4.disabled = true
			$BiteArea/DD5.disabled = true
			$BiteArea/DD6.disabled = true
			
			$BiteArea/LL1.disabled = true
			$BiteArea/LL2.disabled = true
			$BiteArea/LL3.disabled = true
			$BiteArea/RR1.disabled = false
			$BiteArea/RR2.disabled = false
			$BiteArea/RR3.disabled = false
		elif velocity.x < 0 and true_die == false and abs(dest.x - position.x) > abs(dest.y - position.y):  
			$Anim.play("L")
			$HP_bar.rect_position.x = -98
			direction = 2
			$Coll.disabled = true
			$Coll2.disabled = true
			$Coll3.disabled = true
			...
			$CollL.disabled = false
			$CollL2.disabled = false
			###### Байт
			$BiteArea/DD1.disabled = true
			$BiteArea/DD2.disabled = true
			$BiteArea/DD3.disabled = true
			...
			$BiteArea/LL1.disabled = false
			$BiteArea/LL2.disabled = false
			$BiteArea/LL3.disabled = false
			$BiteArea/RR1.disabled = true
			$BiteArea/RR2.disabled = true
			$BiteArea/RR3.disabled = true
		elif velocity.y < 0 and true_die == false:
			$Anim.play("U")
			$HP_bar.rect_position.x = -78
			direction = 2
			$CollR.disabled = true
			$CollR2.disabled = true
			$CollL.disabled = true
			...
			$Coll3.disabled = false
			###### Байт
			$BiteArea/DD1.disabled = false
			$BiteArea/DD2.disabled = false
			$BiteArea/DD3.disabled = false
			$BiteArea/DD4.disabled = false
			$BiteArea/DD5.disabled = false
			$BiteArea/DD6.disabled = false
