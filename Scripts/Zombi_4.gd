...
var d_speed = 60      ####Скорость коотрая включается если Зомби обноружил игрока 
...
func searc_for_target():          #поиск цели
	...
	if pl: ## Если есть кого доставать (чтобы игра не ломалась после смерти ГГ)
		...
		######Чтобы понять с какого расстояния стрелять шипами
		if target and true_die == false and shot3 == true:             
			if global_position.distance_to(pl.global_position) < 1100:  	
				shot1 = true 
				shot2 = true
				shot3 = false
			else:
