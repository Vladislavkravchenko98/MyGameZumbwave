...
var Bar = true   #Переменная для включения Health_Bara (используется на строке 70)
signal on_death
...
func _process(delta):
	...
	position.x = clamp(position.x, 1000, 6410)
	position.y = clamp(position.y, 0, 3596)
	
	if Bar == true:                                        # Для включения Health_Bar в самом начале игры (70 строка)
		...
		if allow_shoot == true and number_shoot == true and ShotAndBarrvalue == true and ShotAndGunvalue == true:
			...
			if number_of_bullets - 1 != 0 and number_of_bullets > 0: 
				...
				if inc_ammo > 0:
					...
					number_of_bullets = 20
					inc_ammo = inc_ammo - 1
					if yoy1 in inventory.keys():  #Если есть в инвентаре Ammo, то уменьшаем значение влуте и добавляем в UI значение
