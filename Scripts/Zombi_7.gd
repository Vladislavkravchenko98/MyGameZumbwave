...
var d_speed = 110      ####Скорость коотрая включается если Зомби обноружил игрока 

var true_die = false ######################## Перменная отвечающая за то жив или мертв Зомби


##Переменные кот определют что зомби может атаковать
var bite_strength = 0  # Сколько HP БУДЕТ ОТНИМАТЬ ЗОМБИ У ИГРОКА ПРИ ОДНОМ УКУСЕ
...
func bite_barr(targ_br):
	targ_br.reduce_hp(25)            #####Функция отвечающая за байт
	can_bite = false
	#Запуск таймера
...
const PLASMA = preload("res://Scenes/Boom.tscn")
func Shot():
